...
- 彻底清理并移除旧的mock系统
  - 删除 `src/services/mockData.ts` 文件，完全迁移到新的mock系统
  - 更新 `src/services/datasource.ts`，使用新mock系统的数据源
  - 修复旧mock数据与新系统之间的类型不兼容问题
  - 完全消除对旧mock系统的依赖
  - 移除 `public/api` 目录下的静态mock文件
  - 移除 `public` 目录下的swagger相关文件和mock响应文件
  - 清理项目根目录，移除不必要的文件：
    - 移除旧的后端服务目录 `webService`
    - 移除启动脚本 `start-with-mock.sh`
    - 移除简化版配置文件 `vite.simple.js`
    - 移除重复的 `webview-ui` 目录
    - 整理所有Vite时间戳配置文件到备份目录
  - 彻底删除所有临时文件和备份：
    - 删除临时备份目录 `temp_backups` 及其所有内容
    - 整理文档目录结构，分类存放API工具和Swagger文件
    - 保持项目目录结构清晰简洁，仅保留必要文件
- 解决类型兼容性问题
  - 添加类型适配器函数，统一不同模块间的类型定义
  - 使用标准类型导入语法，避免编译错误
  - 确保类型安全检查通过，消除潜在的运行时错误
- 完成旧mock系统的最终验证
  - 通过实际运行验证所有API请求成功拦截和处理
  - 确认数据源、查询和集成API正常工作
  - 验证没有任何前端代码需要修改，保持了界面功能一致性
  - 确认所有API路径格式化功能正常，防止路径重复
- 完成旧mock系统到新mock系统的迁移
  - 保留了旧系统的所有功能和数据
  - 在新系统中创建了完整的兼容层
  - 让两套系统能够协同工作，提高了系统稳定性
  - 简化了未来的维护，只需要维护一套模拟数据
  - 删除了src/services/mockData.ts文件，完全迁移到新的mock/data目录
  - 修复了所有引用旧mock数据的服务文件，如query.ts和versionService.ts
  - 更新了serverMock.ts文件以使用新的mock服务，并添加了废弃警告
- 完全清理了旧mock系统的残留代码和文件
  - 删除 `src/services/mockData.ts` 文件
  - 修改 `src/services/datasource.ts` 使用新的mock数据导入
  - 统一了数据源类型定义，解决类型不兼容问题
  - 确保所有mock数据来源一致，提高系统稳定性
- 完成旧mock代码的最终清理和验证
  - 保留vite.config.ts中对已删除文件的排除配置，确保构建安全性
  - 验证所有API请求均由新系统正确处理
  - 通过实际运行验证系统稳定性，确认无遗留问题
  - 确保开发体验一致性，使开发人员能继续使用相同的工作流程
  - 验证前端界面在整个迁移过程中功能保持完整，无需修改前端实现
