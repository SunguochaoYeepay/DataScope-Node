...
class MetadataService {
     ...
     * @returns 同步结果
     */
    async syncMetadata(dataSourceId, options = {}) {
        ...
        try {
            ...
            logger_1.default.info(`开始同步数据源元数据 [${dataSourceId}], 同步历史ID: ${syncHistoryId}`);
            // 获取数据源连接器
            const connector = await datasource_service_1.default.getConnector(dataSourceId);
            // 1. 先获取数据库架构列表
            const schemas = await connector.getSchemas();
            logger_1.default.info(`发现 ${schemas.length} 个数据库架构: ${schemas.join(', ')}`);
            for (const schemaName of schemas) {
                // 检查schema名称是否匹配模式
                if (options.schemaPattern && !new RegExp(options.schemaPattern).test(schemaName)) {
                    logger_1.default.debug(`跳过架构: ${schemaName}, 不符合模式: ${options.schemaPattern}`);
                    continue;
                }
                // 2. 创建或更新Schema记录
                ...
                if (!schema) {
                    ...
                    logger_1.default.debug(`创建新架构: ${schemaName}, ID: ${schema.id}`);
                }
                else {
                    ...
                    logger_1.default.debug(`更新现有架构: ${schemaName}, ID: ${schema.id}`);
                }
                // 3. 获取表列表
                const tables = await connector.getTables(schemaName);
                logger_1.default.info(`在架构 ${schemaName} 中发现 ${tables.length} 个表/视图`);
                for (const tableInfo of tables) {
                    // 检查表名是否匹配模式
                    if (options.tablePattern && !new RegExp(options.tablePattern).test(tableInfo.name)) {
                        logger_1.default.debug(`跳过表: ${tableInfo.name}, 不符合模式: ${options.tablePattern}`);
                        continue;
                    }
                    // 表类型 TABLE 或 VIEW
                    ...
                    if (!table) {
                        ...
                        logger_1.default.debug(`创建新表: ${tableInfo.name}, ID: ${table.id}`);
                    }
                    else {
                        ...
                        logger_1.default.debug(`更新现有表: ${tableInfo.name}, ID: ${table.id}`);
                    }
                    // 如果是全量同步，先删除旧的列记录
                    if (syncType === 'FULL') {
                        ...
                        logger_1.default.debug(`删除表 ${tableInfo.name} 的所有列记录`);
                    }
                    // 5. 获取表的列信息
                    const columns = await connector.getColumns(schemaName, tableInfo.name);
                    logger_1.default.info(`在表 ${tableInfo.name} 中发现 ${columns.length} 个列`);
                    // 6. 获取主键信息
                    ...
                    for (const columnInfo of columns) {
                        ...
                        // 判断是否为外键
                        const isForeignKey = foreignKeys.some(fk => fk.columnName === columnInfo.name);
                        // 创建或更新列记录
                        ...
                        if (!column) {
                            ...
                            logger_1.default.debug(`创建新列: ${columnInfo.name}, ID: ${column.id}`);
                        }
                            ...
                            logger_1.default.debug(`更新现有列: ${columnInfo.name}, ID: ${column.id}`);
                        }
                    }
                    // 9. 处理外键关系
                    if (syncType === 'FULL') {
                        ...
                        logger_1.default.debug(`删除表 ${tableInfo.name} 的所有关系记录`);
                    }
                    // 处理新的外键关系
                    for (const fk of foreignKeys) {
                        ...
                        if (!targetSchema) {
                            logger_1.default.warn(`找不到目标架构: ${targetSchemaName}, 跳过外键关系`);
                            continue;
                        }
                        // 寻找目标表
                        ...
                        if (!targetTable) {
                            logger_1.default.warn(`找不到目标表: ${fk.referencedTable}, 跳过外键关系`);
                            continue;
                        }
                        // 寻找源列和目标列
                        ...
                        if (!sourceColumn || !targetColumn) {
                            logger_1.default.warn(`找不到外键关系的源列或目标列, 跳过此关系`);
                            continue;
                        }
                        // 创建表关系
                        ...
                        logger_1.default.debug(`创建外键关系: ${fk.constraintName} (${table.name}.${sourceColumn.name} -> ${targetTable.name}.${targetColumn.name})`);
                    }
                }
            }
            // 使用关系检测器检测表关系
            ...
            logger_1.default.error(`数据源元数据同步失败 [${dataSourceId}]`, { error });
            // 更新同步历史记录为失败状态
            if (syncHistoryId) {
                await prisma.metadataSyncHistory.update({
                    where: { id: syncHistoryId },
            ...
            throw new error_1.ApiError(`数据源元数据同步失败: ${error.message}`, 500, { dataSourceId, error: error.stack });
        }
    }
    /**
     * 获取数据源的元数据结构
     ...
     * @returns 元数据结构
     */
    async getMetadataStructure(dataSourceId) {
        try {
            // 获取该数据源下的所有Schema
            ...
            if (schemas.length === 0) {
                throw new error_1.ApiError('数据源元数据未同步', 404);
            }
            return schemas;
        }
        catch (error) {
            logger_1.default.error('获取元数据结构失败', { error, dataSourceId });
            if (error instanceof error_1.ApiError) {
                throw error;
            }
            throw new error_1.ApiError('获取元数据结构失败', 500, { message: error?.message || '未知错误' });
        }
    }
    /**
     * 获取同步历史记录
     ...
     * @returns 同步历史记录
     */
    async getSyncHistory(dataSourceId, limit = 10, offset = 0) {
        try {
            ...
            logger_1.default.error('获取同步历史失败', { error, dataSourceId });
            throw new error_1.ApiError('获取同步历史失败', 500, { message: error?.message || '未知错误' });
        }
    }
    /**
     * 获取表数据预览
     ...
     * @returns 表数据预览
     */
    async previewTableData(dataSourceId, schemaName, tableName, limit = 100) {
        try {
            // 获取数据源连接器
            const connector = await datasource_service_1.default.getConnector(dataSourceId);
            // 预览表数据
            ...
            logger_1.default.error('获取表数据预览失败', { error, dataSourceId, schemaName, tableName });
            if (error instanceof error_1.ApiError) {
                throw error;
            }
            throw new error_1.ApiError('获取表数据预览失败', 500, { message: error?.message || '未知错误' });
        }
    }
}
