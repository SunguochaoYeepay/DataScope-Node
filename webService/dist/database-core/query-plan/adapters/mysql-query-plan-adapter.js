...
exports.MySQLQueryPlanAdapter = void 0;
/**
 * MySQL查询计划适配器
 * 用于转换MySQL的EXPLAIN结果为统一的QueryPlan格式
 */
class MySQLQueryPlanAdapter {
    /**
     ...
     * @returns 统一格式的查询执行计划
     */
    static convert(traditionalRows, jsonData, originalQuery) {
        const planNodes = [];
        // 处理传统格式的执行计划行
     ...
     * @returns 优化建议列表
     */
    static generateOptimizationTips(planNodes) {
        const tips = [];
        // 检查表扫描
        ...
        // 检查临时表和文件排序
        const fileSort = planNodes.some((node) => node.extra && node.extra.includes('Using filesort'));
        const tempTable = planNodes.some((node) => node.extra && node.extra.includes('Using temporary'));
        if (fileSort) {
...
exports.MySQLQueryPlanAdapter = MySQLQueryPlanAdapter;
//# sourceMappingURL=mysql-query-plan-adapter.js.map