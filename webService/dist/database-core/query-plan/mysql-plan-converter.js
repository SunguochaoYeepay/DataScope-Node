...
class MySQLQueryPlanConverter {
     ...
     * @returns 统一格式的查询计划
     */
    static convertToQueryPlan(traditionalRows, jsonData, originalQuery) {
        const planNodes = [];
        // 处理传统格式的执行计划行
     ...
     * @returns 提取的性能分析数据
     */
    static extractPerformanceData(jsonData) {
        try {
            ...
            logger_1.default.warn('从JSON执行计划提取性能数据失败', { error: error.message });
            return {};
        }
    }
    /**
     ...
     */
    static traverseQueryBlock(node, performanceData) {
        // 处理表对象
        if (node.table) {
            // 收集使用的索引
        ...
        // 递归处理嵌套查询
        if (node.nested_loop) {
            node.nested_loop.forEach((loop) => this.traverseQueryBlock(loop, performanceData));
        }
        // 处理子查询
        if (node.subqueries) {
            node.subqueries.forEach((subquery) => this.traverseQueryBlock(subquery, performanceData));
        }
        // 处理UNION查询
        if (node.union_result) {
            if (node.union_result.query_specifications) {
                node.union_result.query_specifications.forEach((spec) => this.traverseQueryBlock(spec, performanceData));
            }
        }
    }
    /**
     ...
     * @returns 优化建议数组
     */
    static generateOptimizationTips(plan) {
        const tips = [];
        // 检查表扫描
...
//# sourceMappingURL=mysql-plan-converter.js.map