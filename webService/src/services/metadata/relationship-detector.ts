...
export class RelationshipDetector {
   ...
   * @returns 检测到的关系数量
   */
  async detectRelationships(dataSourceId: string): Promise<number> {
    let detectedCount = 0;
    
    try {
      logger.info(`开始检测数据源 ${dataSourceId} 的表关系`);
      ...
      logger.info(`数据源 ${dataSourceId} 关系检测完成，共检测到 ${detectedCount} 个关系`);
      
      return detectedCount;
    } catch (error) {
      logger.error(`检测表关系时出错`, { error, dataSourceId });
      throw error;
    }
  }
   ...
   * @returns 检测到的关系数量
   */
  private async detectForeignKeyRelationships(dataSourceId: string): Promise<number> {
    let count = 0;
    ...
    try {
      ...
      logger.info(`找到 ${foreignKeyColumns.length} 个外键列`);
      ...
      for (const fkColumn of foreignKeyColumns) {
        ...
        for (const fk of foreignKeys as any[]) {
          ...
          if (!existingRelationship) {
            ...
            count++;
            logger.debug(`创建外键关系: ${fkColumn.table.name}.${fkColumn.name} -> ${targetTable.name}.${targetColumn.name}`);
          }
        }
      }
      
      logger.info(`基于外键检测到 ${count} 个关系`);
      return count;
    } catch (error) {
      logger.error(`基于外键检测关系时出错`, { error, dataSourceId });
      return 0;
    }
  }
   ...
   * @returns 检测到的关系数量
   */
  private async detectNamingConventionRelationships(dataSourceId: string): Promise<number> {
    let count = 0;
    ...
    try {
      ...
      for (const sourceTable of tables) {
        ...
        for (const targetTable of tables) {
          ...
          if (existingRelationship) continue; // 跳过已有关系
          ...
          if (potentialRelations.length > 0) {
            ...
            count++;
            logger.debug(`基于命名检测到关系: ${sourceTable.name}.${bestRelation.sourceColumn.name} -> ${targetTable.name}.${bestRelation.targetColumn.name} (置信度: ${bestRelation.confidence})`);
          }
        }
      }
      
      logger.info(`基于命名规则检测到 ${count} 个关系`);
      return count;
    } catch (error) {
      logger.error(`基于命名规则检测关系时出错`, { error, dataSourceId });
      return 0;
    }
  }
}

export default new RelationshipDetector();