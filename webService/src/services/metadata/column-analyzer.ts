...
export class ColumnAnalyzer {
  ...
  async analyzeColumn(
    ...
    try {
      logger.info(`开始分析列 ${schemaName}.${tableName}.${columnName}`);
      ...
      if (['int', 'bigint', 'decimal', 'float', 'double', 'date', 'datetime', 'timestamp'].includes(column.dataType.toLowerCase())) {
        distribution = await this.getDataDistribution(
          connector,
          schemaName,
          tableName,
          columnName,
      ...
      // 获取样本值
      const samples = await this.getSampleValues(
        connector,
        schemaName,
        tableName,
        columnName
      ...
      if (!column.isPrimaryKey) {
        uniqueCount = await this.getUniqueValuesCount(
          connector,
          schemaName,
          tableName,
          columnName
      ...
      logger.info(`列 ${schemaName}.${tableName}.${columnName} 分析完成`);
   ...
   * 获取数据分布信息
   */
  private async getDataDistribution(
    connector: any,
    schemaName: string,
    tableName: string,
    columnName: string,
   ...
   * 获取列的唯一值数量
   */
  private async getUniqueValuesCount(
    connector: any,
    schemaName: string,
    tableName: string,
    columnName: string
...
export default new ColumnAnalyzer();