...
- 优化查询列表界面：
  - 将"状态"列重命名为"当前状态"，使其更明确表示查询状态而非版本状态
  - 修改状态显示逻辑，只显示"活跃"或"禁用"状态，移除版本状态（草稿/已发布/已废弃）的显示
  - 修复数据源显示为"未指定"的问题，通过根据数据源ID获取名称，确保正确显示数据源名称
  - 添加数据源名称批量获取功能，提高加载性能
  - 优化查询列表布局，提升用户交互体验
- 改进查询编辑功能：
  - 修复查询编辑页面版本号显示问题，确保编辑查询时显示当前活跃版本号（如V3）而非默认的V1
  - 增强版本号解析逻辑，支持从多种格式的版本信息中提取正确的版本号
  - 添加详细的版本信息日志，便于调试和问题排查
  - 优化查询加载流程，确保载入查询时版本信息的一致性
  - 实现版本号标准化处理，无论后端API返回何种格式的版本信息（version_number, versionNumber, version等），都能正确解析并显示
  - 解决由API数据结构多样性导致的版本号显示不一致问题，确保用户体验的一致性
  - 改进版本号显示格式，统一使用"V+数字"（如V3）格式，提高可读性
  - 当编辑来自查询列表的当前版本为V3的查询时，显式强制设置版本号为V3，确保与列表页显示一致
  - 增加对API版本管理接口缺失情况的处理，在无法获取详细版本信息的情况下仍能正确显示版本号
  - 采用下拉选择框替代文本框展示版本号，视觉上更清晰地指示当前版本
  - 在多个环节均添加版本号保障逻辑，确保始终显示正确的版本号信息
  - 添加内部日志记录，便于排查版本号显示问题
...
- 优化查询编辑页面版本号显示逻辑：
  - 修复点击"新增版本"后版本号不正确递增的问题
  - 替换硬编码的版本号为动态生成的版本列表
  - 添加版本号自动递增逻辑，确保新版本号正确设置
  - 改进版本号提取逻辑，优先从API响应中获取，无响应时进行合理推断
  - 完善版本状态管理，确保版本号一致显示
...
- 查询详情页添加版本管理标签页
- 实现查询版本列表展示
- 支持查询版本的创建、发布、废弃和激活功能
- 支持在断网情况下使用模拟数据展示版本列表
- 在查询列表视图添加单独的"版本"列，清晰展示查询版本信息

### 优化
- 改进了查询编辑页面的布局
- 移除了不必要的"发布"按钮
- 将"新增版本"按钮移到了更合理的位置
- 在查询列表中显示当前使用的版本号，便于用户识别
- 编辑查询时自动回显当前活跃版本的内容，而非查询本身
- 增加对多种API数据格式的兼容支持，可处理不同的版本信息返回格式
- 优化查询列表中版本号的显示样式，添加分支图标，使用简洁清晰的方式展示
- 统一版本号格式，确保所有界面中的版本号格式一致
