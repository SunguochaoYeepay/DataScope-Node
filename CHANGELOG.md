...
- 修复前端查询历史API调用路径错误问题
  - 修正了`getQueryExecutionHistory`方法，调整API路径为`/api/queries/history?queryId={queryId}`
  - 增强了响应处理逻辑，支持多种后端返回的数据结构
  - 添加了客户端过滤功能，确保只返回与指定查询ID相关的历史记录
  - 完善了日志记录，便于调试和排错
  - 修改了`stores/query.ts`中的`getQueryExecutionHistory`方法，使用服务层方法替代直接API调用，保持代码一致性
  - 增强了后端`getQueryHistory`控制器方法，添加对queryId参数的支持，使前端可以通过查询参数精确获取特定查询的历史记录
  - 完善了后端日志记录，详细记录查询条件和结果数量

- 修复前端API调用路径错误问题
  - 修正了`getQueryExecutionHistory`方法，将API路径从`/api/queries/{queryId}/execution-history`修改为正确的`/api/queries/{queryId}/history`
  - 修正了`getQueryExecutionPlan`方法，调整API路径为`/api/queries/plans`并添加queryId作为查询参数
  - 优化了API响应处理，针对数组型响应提取第一条记录
  - 增强了错误处理，确保不符合预期的API响应不会导致前端崩溃
  - 完善了日志记录，更清晰地记录API调用和响应数据
